...
var Touchy = {
	...
	wheel: function(element, options){
		Touchy.jQuery(element).click(function(event){
	...
	wheelOnFire: function(event, options){
		...
		var winkel = Math.PI * 1.9 / circles.length;
		var radiusBig = 70;
		var radiusSmall = (2 * Math.PI * radiusBig) / circles.length / 2.3;
		...
		for(var i = 0; i < circles.length; i++){
			...
			var tt = s.text(-100, -100, circles[i]);
			var t = s.text(startX + x - (tt.getBBox().width / 2), startY + y + (tt.getBBox().height / 4), circles[i]);
			...
			g.click(function(){
				var current = this;
				var i = 0;
				Touchy.jQuery(this.parent().node).find("g").each(function(v, k){
					if(current.node.isEqualNode(k))
						return true;
				...
				current.parent().select("circle").animate({"fill-opacity": 0}, 100);
				
				window.setTimeout(function(){
					current.select("circle").animate({r: 0}, 200, null, function(){
						this.parent().parent().remove();
						this.remove();
					});
					current.select("text").animate({"fill-opacity": 0}, 100, null, function(){ });
				}, 500);
			});

			window.setTimeout(function(lc){
				lc.animate({r: radiusSmall}, 300);
			}, i * 100, c);
